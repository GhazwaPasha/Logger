...
| **manager** | Bound to **one or more departments** via **`organization_member_managed_departments`** (API: **`managedDepartmentIds`**); legacy **`departmentId`** on the member row is the first level; sees all tasks in lists under any managed department |
| **member** | Org member; task visibility driven by assignment (see below) |

**Invite/upsert** (owner-only API): **`POST /organizations/:id/members`** with email + role + **`departmentIds`** (non-empty for managers) or legacy **`departmentId`** (single level).

## Task visibility (`listTasksForUser`)

- **Owner:** all tasks in org (`deletedAt` null).
- **Manager:** tasks whose **`listId`** is in lists whose **`departmentId`** is in the manager’s **`managedDepartmentIds`** set (junction table; fallback to **`departmentId`** if junction empty).
...
User may open a task if **owner**, **dept manager for task’s list’s department** (list’s department ∈ managed set), or **assignee**. Otherwise **403**.
