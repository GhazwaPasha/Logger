...
- API allows browser origins from **`API_CORS_ORIGINS`** (comma-separated) when set; otherwise each origin in **`NEXT_PUBLIC_APP_URL`** (comma-separated, same as issuer fallback), plus localhost regex and Expo `exp://`. The allowed set must include the **actual `Origin` header** users’ browsers send (custom domain vs `*.vercel.app` vs `www` vs apex); mismatch → CORS block → client **`Failed to fetch`** on workspace/API calls even though the server process is healthy.
