...
function StatTile({
  ...
  return (
    <div
      className="ui-elevated-panel min-w-0 rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-elevated)] p-2.5"
      aria-busy={loading || undefined}
    >
      <p className="text-[11px] font-medium uppercase tracking-wide text-[var(--muted)]">{label}</p>
      {loading ? (
        <div
          className="mt-1.5 h-7 w-10 animate-pulse rounded-md bg-[var(--surface-muted)] motion-reduce:animate-none"
          aria-hidden
        />
      ) : (
        <p
